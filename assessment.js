...
var secondHero = heroes.slice(2, 3)
...
function doubleTheFun(whatever) {
  if(isNaN(whatever)){
    return whatever + whatever
  }
  else {
    return parseFloat(whatever) + parseFloat(whatever)
  }
}
...
// #14 Create a function called coderTest that takes in an object that is a person.
  // It looks to see if the person’s name is Jeremy and then changes the person object
  // to have a property called lovesCode with a value of 10.  If their name is Brack set lovesCode to 0.  otherwise set lovesCode to 5.
...
function outside(temp, hum, cloud){
  if(temp > 80 && hum > 40) return "I'm all sweat";
  else if(temp < 40 && cloud > 60) return "I have icecicles";
  else if(temp > 80 && hum < 40 && cloud < 20) return "I'm literally in the desert";
  else if(temp > 80 || hum > 50 || cloud > 50) return "Hmm, probably not";
